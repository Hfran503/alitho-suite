...
:: MAKE SOME CODE
ECHO Generating Stub Code: PaceUtilSampleCompany.cs
Call :SubMakeParms %3
wsdl /username:%1 /password:%2 /parameters:parms.xml  1>>stubsgen.log

ECHO Moving the PaceUtilSampleCompany.cs to the project folder
move /Y PaceUtilSampleCompany.cs ./..
...
echo   ^<language^>CS^</language^> >> parms.xml
...
echo   ^<out^>PaceUtilSampleCompany.cs^</out^> >> parms.xml
echo     ^<namespace^>efipaceservices.samplecompany^</namespace^> >> parms.xml

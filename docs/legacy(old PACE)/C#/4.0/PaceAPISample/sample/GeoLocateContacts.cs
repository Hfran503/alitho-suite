...
namespace Pace_Web_Service_SDK.sample
{
    class GeoLocateContacts
    {
        ...
        {
            GeoLocateHttpBinding geoLocateHttpBinding = PaceClient.getGeoLocateHttpBinding();
            String[] keys = geoLocateHttpBinding.findContacts(latitude, longitude, radius, xpathExpression);

...
namespace Pace_Web_Service_SDK.sample
{
    class FindObjects
    {
        public static void Run()
        {
            ...
            String[] keys = findObjectsHttpBinding.findAndSort("Job", "adminStatus/@openJob", sort);

            //Date Filter Sample
            String[] keys2 = findObjectsHttpBinding.find("Job", "@dateSetup = date( 2012, 07, 03 )");

            Console.WriteLine(keys2.Length + " Jobs Setup As Of 2012-07-03");

            //Finds ALL Job Shipments for a particular job, part, with a not null tracking number
            String[] shipments = findObjectsHttpBinding.find("JobShipment", " ( @job ='090005' and @jobPart ='01' ) and @trackingNumber !=''");

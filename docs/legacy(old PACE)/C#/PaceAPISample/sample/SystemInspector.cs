...
namespace Pace_Web_Service_SDK.sample
{
    class SystemInspector
    {
        ...
        {
            SystemInspectorPortType systemInspectorHttpBinding = PaceClient.getSystemInspectorHttpBinding();
            /**
            ...
            try
            {
                ArrayOfString2 keys = systemInspectorHttpBinding.inspect("com.pace2020.appbox.web.form.ValidFormInspectionTarget[ePace Mobile]");
                Console.WriteLine(keys.Count + " Messages");
            }
            catch (Exception e)
            {

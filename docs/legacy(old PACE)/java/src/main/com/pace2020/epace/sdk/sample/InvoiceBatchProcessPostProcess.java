...
public class InvoiceBatchProcessPostProcess extends SecuredWSDLClient
{
    public static void main( final String[] args ) throws Exception
    {
        new InvoiceBatchProcessPostProcess().run();
    }
    ...
    public boolean run() throws RemoteException
    {
        try
        {
            final ProcessResults results = getInvokeProcessHttpPort().invoiceBatchPostSingle( "5322" );

            System.out.println( " Invoice Batch Process Post " + ( results.isSuccessful() ? "success" : "failure" ) );

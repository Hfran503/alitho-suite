...
public class InvoiceBatchPostSingleProcess extends SecuredWSDLClient
{
    public static void main( final String[] args ) throws Exception
    {
        new InvoiceBatchPostSingleProcess().run();
    }
    ...
    public boolean run() throws RemoteException
    {
        try
        {
            final ProcessResults results = getInvokeProcessHttpPort().invoiceBatchPostSingle( "10001" );

            System.out.println( " Invoice Batch Post Single " + ( results.isSuccessful() ? "success" : "failure" ) );

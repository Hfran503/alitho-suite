...
public class PayrollBatchCalculateProcess extends SecuredWSDLClient
{
    public static void main( final String[] args ) throws Exception
    {
        new PayrollBatchCalculateProcess().run();
    }
    ...
    public boolean run() throws RemoteException
    {
        try
        {
            final ProcessResults results = getInvokeProcessHttpPort().payrollBatchCalculate( "@payrollBatchID = 5001" );

            System.out.println( " Payment Batch Calculate : " + ( results.isSuccessful() ? "success" : "failure" ) );

...
import type { NetsuitePOLineJobData } from './netsuite-po-line-queue'
...
const startWorker = async () => {
    ...
    async (job: Job<NetsuitePOLineJobData>) => {
      ...
      try {
        ...
        if (result.success) {
          ...
          throw new Error(result.error || 'Failed to send PO Line to NetSuite')
        }
      } catch (error) {
  ...
  console.log('⏳ Waiting for PO Line jobs...')
...
startWorker().catch((error) => {
  console.error('Failed to start NetSuite PO Line Worker:', error)
  process.exit(1)
})

// Graceful shutdown
process.on('SIGTERM', () => {
  console.log('👋 Shutting down NetSuite PO Line Worker...')
  process.exit(0)
})

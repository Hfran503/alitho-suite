...
import type { NetsuiteInvoiceJobData } from './netsuite-invoice-queue'
...
const startWorker = async () => {
  ...
  console.log('⏳ Waiting for invoice jobs...')
...
startWorker().catch((error) => {
  console.error('Failed to start NetSuite Invoice Worker:', error)
  process.exit(1)
})

// Graceful shutdown
process.on('SIGTERM', () => {
  console.log('👋 Shutting down NetSuite Invoice Worker...')
  process.exit(0)
})

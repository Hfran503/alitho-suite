    ...
    <div className="p-4 max-w-7xl mx-auto">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        ...
        <div className="lg:col-span-2 space-y-4">
          ...
          {rateMode === 'batch' && (
            <>
              {/* From Address for Batch */}
              <div className="bg-white rounded-lg shadow p-4">
                <div className="mb-3">
                  <div className="flex items-center justify-between">
                    <h2 className="text-sm font-bold text-gray-900">From Address (Origin)</h2>
                    {loadingDefaults && (
                      <span className="text-xs text-gray-500 italic">Loading defaults...</span>
                    )}
                  </div>
                ...
                {loadingMappings ? (
                  ...
                  <div className="text-xs text-amber-600 bg-amber-50 border border-amber-200 rounded p-3">
                    <div className="flex items-start gap-2">
                      ...
                      <div>
                        <p className="font-medium">No carrier services configured</p>
                        <p className="mt-0.5">Please configure carrier service mappings in Settings → Integrations → ShipStation</p>
                      </div>
                    </div>
                  </div>
                ) : (
                  <div>
                    ...
                    {selectedService && (
                      <p className="text-xs text-gray-500 mt-1.5">
                        Selected: {carrierServiceMappings.find(
                          (m) => `${m.shipstationCarrierId}|${m.shipstationServiceCode}` === selectedService
                        )?.carrierName} - {carrierServiceMappings.find(
                          (m) => `${m.shipstationCarrierId}|${m.shipstationServiceCode}` === selectedService
                        )?.serviceName}
                      </p>
                    )}
              ...
              {/* Destinations List - Compact View */}
              <div className="bg-white rounded-lg shadow p-4">
                <div className="flex items-center justify-between mb-3">
                  <div>
                    <h2 className="text-sm font-bold text-gray-900">
                      Destinations ({batchDestinations.length})
                      {batchResults.length > 0 && (
                        <span className="ml-2 text-xs font-normal text-gray-500">
                          {batchResults.filter(r => r.status === 'success').length} quoted
                        </span>
                      )}
                    </h2>
                    {selectedService && carrierServiceMappings.length > 0 && (
                ...
                {batchDestinations.length === 0 ? (
                  <div className="text-center py-6 border-2 border-dashed border-gray-200 rounded-lg">
                    ...
                    <p className="text-xs text-gray-500">No destinations added yet</p>
                    <p className="text-xs text-gray-400 mt-1">Click "Add" or "Import" a CSV/Excel file</p>
                    <div className="mt-3 pt-3 border-t border-gray-100 mx-8">
                      <button
                        onClick={downloadTemplate}
                        className="inline-flex items-center gap-1 text-xs text-blue-600 hover:text-blue-800 font-medium mb-2"
                      >
                        <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4" />
                        </svg>
                      ...
                      <p className="text-xs text-gray-400">
                        Required: street1, city, state, zip, weight
                      </p>
                      <p className="text-xs text-gray-300 mt-0.5">
                        Optional: name, company, residential, length, width, height
                      </p>
                    </div>
                  </div>
                ) : (
                  <div className="space-y-1 max-h-[500px] overflow-y-auto">
                    {/* Table Header */}
                    <div className="grid grid-cols-12 gap-2 px-3 py-2 bg-gray-50 rounded text-xs font-medium text-gray-600 sticky top-0">
                    ...
                    {batchDestinations.map((dest, index) => {
                      ...
                      return (
                        <div key={dest.id} className="border border-gray-200 rounded overflow-hidden">
                          {/* Compact Row View */}
                          <div
                            className={`grid grid-cols-12 gap-2 px-3 py-2 items-center text-xs hover:bg-gray-50 cursor-pointer ${
                              result?.status === 'error' ? 'bg-red-50' : ''
                          ...
                          >
                            {/* Destination Name/Company */}
                            <div className="col-span-3 truncate">
                              <span className="font-medium text-gray-900">
                                {dest.name || dest.company || `#${index + 1}`}
                              </span>
                              {dest.name && dest.company && (
                                <span className="text-gray-500 ml-1">({dest.company})</span>
                              )}
                            </div>
                            ...
                            <div className="col-span-2 text-center">
                              {result?.status === 'loading' ? (
                                ...
                                <div className="inline-flex items-center gap-1 px-2.5 py-1 bg-green-500 text-white rounded font-bold shadow-sm">
                                  ${result.cheapestRate.amount.toFixed(2)}
                                  {result.cheapestRate.deliveryDays && (
                                    <span className="text-[10px] font-normal opacity-90">
                                      ({result.cheapestRate.deliveryDays}d)
                          ...
                          {isExpanded && (
                            <div className="px-3 py-3 bg-gray-50 border-t border-gray-200">
                              <div className="grid grid-cols-4 gap-2">
                                <div className="col-span-2">
                                  <label className="block text-xs font-medium text-gray-700 mb-1">Name/Company</label>
                                  <input
                                    type="text"
                                    value={dest.name}
                                    onChange={(e) => updateBatchDestination(dest.id, 'name', e.target.value)}
                              ...
                              {/* Rate Breakdown if available */}
                              {result?.status === 'success' && result.cheapestRate && (
                                <div className="mt-3 pt-3 border-t border-gray-200">
                                  <div className="flex items-center justify-between">
                                    <div className="text-xs text-gray-600">
                                      <span className="font-medium">Rate Breakdown:</span>
                                      {result.cheapestRate.shippingAmount !== undefined && (
                                        <span className="ml-2">Shipping: ${result.cheapestRate.shippingAmount.toFixed(2)}</span>
                                      )}
                                      {result.cheapestRate.markupAmount !== undefined && result.cheapestRate.markupAmount > 0 && (
                                        <span className="ml-2 text-blue-600">+ Markup: ${result.cheapestRate.markupAmount.toFixed(2)}</span>
                                      )}
                                      {result.cheapestRate.processingCost !== undefined && result.cheapestRate.processingCost > 0 && (
                                        <span className="ml-2 text-green-600">+ Processing: ${result.cheapestRate.processingCost.toFixed(2)}</span>
                                      )}
                                    </div>
                  ...
                  <div className="mt-3 pt-3 border-t border-gray-200">
                    <div className="flex items-center justify-between text-xs">
                      <div className="text-gray-600">
                        <span className="font-medium">Total Quotes:</span> {batchResults.filter(r => r.status === 'success').length} of {batchResults.length}
                      </div>
              ...
              <div className="flex gap-2">
                <button
                  onClick={handleBatchQuotes}
                  disabled={batchLoading || batchDestinations.length === 0}
                  className="flex-1 py-2.5 bg-purple-600 text-white rounded hover:bg-purple-700 disabled:opacity-50 disabled:cursor-not-allowed font-medium transition-colors text-sm"
                >
                  {batchLoading ? `Getting Rates... (${batchProgress.completed}/${batchProgress.total})` : 'Get Batch Quotes'}
                </button>
        ...
        <div className="lg:col-span-1">
          ...
          {rateMode === 'batch' ? (
            <div className="bg-white rounded-lg shadow p-4 sticky top-4">
              <h2 className="text-sm font-bold text-gray-900 mb-3">Batch Summary</h2>
              ...
              {batchDestinations.length === 0 ? (
                <div className="text-center py-6">
                  ...
                  <p className="text-xs text-gray-500">Add destinations to get started</p>
                  <p className="text-xs text-gray-400 mt-0.5">Import a file or add manually</p>
                </div>
              ) : (
                <div className="space-y-4">
                  {/* Destination Stats */}
                  <div className="grid grid-cols-2 gap-3">
                    <div className="bg-gray-50 rounded-lg p-3 text-center">
                      <p className="text-2xl font-bold text-gray-900">{batchDestinations.length}</p>
                      <p className="text-xs text-gray-500">Destinations</p>
                    </div>
                    <div className="bg-gray-50 rounded-lg p-3 text-center">
                      <p className="text-2xl font-bold text-gray-900">
                        {batchDestinations.reduce((sum, d) => sum + (parseFloat(d.weight) || 0), 0).toFixed(1)}
                      </p>
                      <p className="text-xs text-gray-500">Total lbs</p>
                    </div>
                  </div>
                  ...
                  {batchResults.length > 0 && (
                    <div className="border-t border-gray-200 pt-4">
                      <h3 className="text-xs font-bold text-gray-700 mb-2">Quote Results</h3>
                      <div className="space-y-2">
                        <div className="flex items-center justify-between text-xs">
                          ...
                          <span className="font-medium">{batchResults.filter(r => r.status === 'success').length}</span>
                        </div>
                        <div className="flex items-center justify-between text-xs">
                          <span className="flex items-center gap-1.5">
                            <span className="w-2 h-2 rounded-full bg-red-500"></span>
                          ...
                          <span className="font-medium">{batchResults.filter(r => r.status === 'error').length}</span>
                        </div>
                        {batchResults.some(r => r.status === 'loading') && (
                          <div className="flex items-center justify-between text-xs">
                            <span className="flex items-center gap-1.5">
                            ...
                            <span className="font-medium">{batchResults.filter(r => r.status === 'loading').length}</span>
                          </div>
                        )}
                      </div>

                      {/* Total Cost */}
                      {batchResults.some(r => r.status === 'success' && r.cheapestRate) && (
                        <div className="mt-4 pt-3 border-t border-gray-200">
                          <div className="flex items-center justify-between">
                            <span className="text-xs text-gray-600">Total Shipping Cost</span>
                  ...
                  <div className="border-t border-gray-200 pt-4">
                    <h3 className="text-xs font-bold text-gray-700 mb-2">Address Types</h3>
                    <div className="flex items-center gap-2 text-xs">
                      <span className="inline-flex items-center gap-1 px-2 py-1 bg-orange-100 text-orange-700 rounded">
                        <span className="font-medium">{batchDestinations.filter(d => d.residential === 'yes').length}</span> Residential
                      </span>
                      <span className="inline-flex items-center gap-1 px-2 py-1 bg-blue-100 text-blue-700 rounded">
                        <span className="font-medium">{batchDestinations.filter(d => d.residential === 'no').length}</span> Commercial
                      </span>
                      <span className="inline-flex items-center gap-1 px-2 py-1 bg-gray-100 text-gray-600 rounded">
                        <span className="font-medium">{batchDestinations.filter(d => d.residential === 'unknown').length}</span> Unknown
                      </span>
                    </div>
                  </div>
                  ...
                  {!batchResults.length && (
                    <div className="border-t border-gray-200 pt-4">
                      <h3 className="text-xs font-bold text-gray-700 mb-2">Quick Tips</h3>
                      <ul className="space-y-1 text-xs text-gray-500">
                        <li className="flex items-start gap-1.5">
                          <span className="text-purple-500 mt-0.5">•</span>
                          Select a carrier/service above
                        </li>
                        <li className="flex items-start gap-1.5">
                          <span className="text-purple-500 mt-0.5">•</span>
                        ...
                        <li className="flex items-start gap-1.5">
                          <span className="text-purple-500 mt-0.5">•</span>
                          Rates appear inline after quoting
                        </li>
                      </ul>
                    </div>
                  )}

...
import {
  saveEasyPostApiKey,
  deleteEasyPostApiKey,
} from '@/lib/secrets'
import { createAuditLog } from '@/lib/audit'

// GET /api/integrations/easypost - Check if EasyPost is configured
export async function GET(_req: NextRequest) {
  try {
    ...
    try {
      ...
      const secretName = `calitho-suite/integrations/easypost/${membership.tenantId}`
      ...
      if (secret.testApiKey) {
        ...
        maskedTestKey = key.length > 15 ? `${key.substring(0, 7)}...${key.substring(key.length - 4)}` : '***configured***'
      }

      if (secret.productionApiKey) {
        hasProductionKey = true
        const key = secret.productionApiKey
        maskedProductionKey = key.length > 15 ? `${key.substring(0, 7)}...${key.substring(key.length - 4)}` : '***configured***'
      }
    } catch (error) {
    ...
    console.error('Get EasyPost integration error:', error)
    return NextResponse.json({ error: 'Internal server error' }, { status: 500 })
  }
}

// POST /api/integrations/easypost - Save EasyPost credentials
export async function POST(req: NextRequest) {
  try {
    ...
    if (enabled) {
      ...
      if (shipstationIntegration?.enabled) {
        return NextResponse.json(
          {
            error:
              'ShipStation integration is currently active. Please disable it before enabling EasyPost.',
          },
          { status: 400 }
        )
      }
    ...
    if (testApiKey || productionApiKey) {
      await saveEasyPostApiKey(membership.tenantId, testApiKey, productionApiKey)
    }
    ...
    const integration = await db.integration.upsert({
      ...
      create: {
        ...
        config: config || {},
        secretName: `calitho-suite/integrations/easypost/${membership.tenantId}`,
      },
      update: {
        enabled: enabled ?? true,
        config: config || {},
    ...
    await createAuditLog({
      action: 'integration.easypost.configured',
    ...
    console.error('Save EasyPost integration error:', error)
    return NextResponse.json(
      { error: 'Failed to save EasyPost credentials' },
      { status: 500 }
    )
  }
}

// DELETE /api/integrations/easypost - Remove EasyPost integration
export async function DELETE(_req: NextRequest) {
  try {
    ...
    await deleteEasyPostApiKey(membership.tenantId)
    ...
    // Create audit log
    await createAuditLog({
      action: 'integration.easypost.removed',
      entityType: 'integration',
      entityId: undefined,
      userId: session.user.id,
      tenantId: membership.tenantId,
    ...
    console.error('Delete EasyPost integration error:', error)
    return NextResponse.json(
      { error: 'Failed to remove EasyPost integration' },
      { status: 500 }
    )
  }
}

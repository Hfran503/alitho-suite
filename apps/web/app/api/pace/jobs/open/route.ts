...
export async function GET() {
  try {
    ...
    const proposalPromises = uniqueProposals.map(async (proposalNumber) => {
      ...
      return { proposalNumber, estimatePrice: null, totalSellPrice: null, estimate: null }
    })
    ...
    // Add proposal estimate price, total sell price, and estimate to each job
    jobs.forEach(job => {

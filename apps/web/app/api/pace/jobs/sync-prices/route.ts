...
export async function POST(request: Request) {
  try {
    ...
    for (const jobData of jobs) {
      const { jobNumber, proposalTotalSellPrice } = jobData

      if (!jobNumber || proposalTotalSellPrice === null || proposalTotalSellPrice === undefined) {
        failedCount++
        errors.push(`Job ${jobNumber}: Missing job number or proposal total sell price`)
        continue
      }

      try {
        // Update the Job's amountToInvoice field in PACE using UpdateObject
        ...
        const updatePayload = {
          ...
          amountToInvoice: proposalTotalSellPrice
        }

        console.log(`Updating job ${jobNumber}:`, { url: updateUrl, payload: updatePayload })
        ...
        if (updateResponse.ok) {
          successCount++
          console.log(`✅ Updated job ${jobNumber} with amountToInvoice: ${proposalTotalSellPrice}`)
        } else {

      ...
      <html>
        <body style="font-family: Arial, sans-serif; line-height: 1.6; color: #333;">
          ...
          <p style="font-size: 12px; color: #6b7280;">
            This is an automated report sent daily at 8:10 AM.<br>
  ...
  return `
    <html>
      <body style="font-family: Arial, sans-serif; line-height: 1.6; color: #333;">
        ...
        <hr style="margin: 30px 0; border: none; border-top: 1px solid #e5e7eb;">
        <p style="font-size: 12px; color: #6b7280;">
          This is an automated report sent daily at 8:00 AM<br>
          Generated by Calitho Suite
        </p>
      </body>
    </html>
...
export async function GET(request: NextRequest) {
  try {
    ...
    // Get only the Calitho tenant (not Demo Company)
    const tenants = await db.tenant.findMany({
      where: {
        status: 'active',
        name: 'Calitho', // Only process Calitho tenant
      },
      select: {
        id: true,
        name: true,
    ...
    if (tenants.length === 0) {
      return NextResponse.json({
        success: true,
        message: 'Calitho tenant not found or not active',
        sent: 0,
      })
    }

...
export function Step4ReviewV2({
  ...
  const handleCreateBatch = async () => {
    ...
    try {
      ...
      // Create ShipEngine batch via new API
      setProgress({ current: 0, total: mappedRows.length, stage: 'Creating ShipEngine batch...' })

      const response = await fetch('/api/batch-import-v2/create', {
        method: 'POST',
      ...
      onStartProcessing(result.data.shipEngineBatchId)
    } catch (err: any) {
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-gray-600">
          Review your import configuration. This will use ShipEngine's native batch API for faster processing.
        </p>
      </div>
        ...
        <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
          <div className="flex items-center gap-3">
            ...
            <div className="flex-1">
              <p className="text-sm font-medium text-blue-900">{progress.stage}</p>
              {progress.total > 0 && (
        ...
        <div className="flex items-start gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-amber-800">
              Once processing starts, shipping labels will be created via ShipEngine's batch API and you will
              be charged for each label. This process uses native batch processing for better performance.
            </p>
          </div>
        </div>
      </div>

...
function getIcon(iconName: string | null, className: string = "w-5 h-5"): React.ReactNode {
  console.log('getIcon called with:', iconName, 'type:', typeof iconName)
  ...
  useEffect(() => {
    async function fetchMenuConfigs() {
      try {
        ...
        console.log('Fetched menu configs:', configs)
        console.log('Open Jobs config:', configs.find(c => c.menuKey === 'open-jobs'))

        // Transform configs to nav items (API already filtered by role)
        const items = transformMenuConfigs(configs)
        console.log('Transformed nav items:', items)
        console.log('Open Jobs item:', items.find(i => i.href === '/open-jobs'))
        setNavItems(items)
      } catch (error) {
